...
async function run(){
 ...
 for(;cycles<600;cycles++){
  ...
  await settle();assert(document.querySelectorAll('tbody tr').length===240,'row count');
  for(const detail of list)detail.open=false;for(const body of bodies)body.remove();bodies=null;
  await settle();assert(document.querySelectorAll('.detail-body').length===0,'closed body retained');
