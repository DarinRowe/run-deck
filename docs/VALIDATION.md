...
## Direct recovery from an unavailable terminal — 2026-09-25

**Behavior and design.** A saved `linked` launch whose tab was no longer in the workspace reproduced the disabled Start / Review terminal state in native Muxy. The new primary action is **Start again**, with separate unavailable-terminal and unconfirmed-launch explanations. One explicit confirmation authorizes a replacement launch; cancellation preserves the saved command and association. `Launchpad.start` owns recovery through the existing Muxy adapter seam, including exclusion, workspace-version checks, fresh storage/tab reads, metadata preservation and persisted opening intent. No process scanner, background task, permission, or storage schema was added. An absent terminal still does not prove process exit, and recovery does not stop old processes or automatically retry uncertain launches.

**Contracts.** Reviewed [Tabs](https://muxy.app/docs/extensions/tabs) (`list`, `switchTo`, and startup-command `open`), [Dialogs](https://muxy.app/docs/extensions/dialogs) (promise resolves a supplied label or null, default/cancel choices), [Permissions](https://muxy.app/docs/extensions/permissions), [Panels](https://muxy.app/docs/extensions/panels), [Events](https://muxy.app/docs/extensions/events), [Lifecycle](https://muxy.app/docs/extensions/lifecycle), and the official [Theme/Sizing guidance](https://github.com/muxy-app/muxy/blob/main/Muxy/Resources/skills/muxy-extension/SKILL.md). Recovery uses existing calls and consent; no new host capability is assumed. Current docs were checked against actual **Muxy 1.6.1**, extending the earlier 1.6.0 baseline. The separate Beta host blocker below is unaffected.

**Build and automated verification.** Checkout `/Users/darinlo/Documents/Projects/run-deck`, based on `6673035`. `node --test tests/launch-recovery.test.mjs` initially failed all 17 new cases with the old association rejection. Final `npm run check`: **165 passed**, zero failures/skips, distribution **119.3 KiB**. Final JS `index-eS9vylu_.js`, SHA-256 `f69221c75f3e42921ece766ed96a5984a2168bd75b397be7572f0e7e6f950c18`; CSS `index-DUU9nb3u.css`, SHA-256 `b0186e18359eecd0ca9d08b9c9a88230333c8e125195f281e4bc828ee28122ab`. Coverage includes missing/unknown/interrupted associations, cancellation, duplicate clicks, changed/removed records, edited command/directory, restored tabs, workspace events (including the same path), disposal, denied new launches and failed intent writes.

With `npm run dev -- --port 5197 --strictPort`, the production fixture at `tests/preview.html?build=1` passed `checkStarterRecovery`, `checkStarterUnknownLaunch` and `checkStarterWorkspaceChange` from `tests/starter-lifecycle.mjs` in English. These establish one launch, zero mutation on cancellation, focus return, the new Terminal action, retained paused live mode and stale-workspace rejection. All seven translations were checked at 320px with no document/dialog horizontal overflow. Inspected [Chinese light at 320px](launch-recovery/zh-light.png) and the native English light panel; the existing theme stylesheet is unchanged.

**Native verification.** Extensions showed enabled DEV path `/Users/darinlo/Documents/Projects/run-deck/dist`. Reloaded and reopened `muxy-ext://run-deck/panel/index.html`; the new labels/action and final-build focus return tied the running panel to this checkout. The original unavailable `dev` association now offered Start again; cancellation left it unchanged. In the run-deck worktree, checked Working directory and created only `Recovery 0925 1857` with `node scripts/muxy-smoke-server.mjs`. Cancelling the first Muxy startup consent created no terminal and displayed Previous launch unconfirmed. Start again → confirm → one-time Allow launched wrapper/server **74123/74124**, port **49362**, token `22d09b9b-3dfd-4533-abf8-f01daf86d5cd`. Terminal selected that exact output from another tab, Open displayed the helper response/PID in Muxy's browser, and Stop removed the service. This first flow used `index-Dy487fWa.js`; the only subsequent runtime edit restored focus after cancelled dialog actions and removed the unused disabled-state flag.

After closing that stopped terminal and reloading the final build, the same saved command showed Previous terminal unavailable. Cancel returned focus to Start again; Space reopened the confirmation and Return chose the default Cancel. Start again → confirm → Allow then launched **78620/78621**, port **50416**, token `bc612cae-12b9-462c-adec-660e0d12991b`; Stop again removed the service. Escape closed the extension's command dialog. The native host confirmation did not dismiss on the automated Escape key, so its keyboard-cancel evidence is Return on default Cancel, not Escape.

**Cleanup.** Both test terminal tabs, the test browser tab and saved test command were removed through Muxy. Independent `ps`/`lsof` checks found none of the four PIDs or listeners on 49362/50416. Closed the browser fixture TaskSpace and stopped Vite; 5197 was clear. Restored the prior run-deck terminal selection and closed its test-opened panel; restored PrintAnvil's original terminal, visible panel, English, live mode and sort. Existing user processes, saved commands and permission rules were retained. No temporary instrumentation remains.

## Muxy Beta 1076 execution blocker — 2026-09-25

**Diagnosis complete; native service inspection remains blocked by the host.** On Muxy **2.0.0-beta-1076** (protocol compatibility 20), Refresh deterministically displays `invalid command: BadRequest`. The enabled unpacked extension resolves to this checkout's `dist/`, based on `6673035`. No Run Deck runtime change is retained.

The installed release tag resolves to upstream commit `f437734efe902168885eef1ec84ce622ffa06927`. Its [consent preflight](https://github.com/muxy-app/muxy/blob/v2.0.0-beta-1076/crates/muxy-app/src/model/extensions/mod.rs#L714-L720) calls `exec_request(call, 0)` for both `exec` and `exec.start`. The [request constructor](https://github.com/muxy-app/muxy/blob/v2.0.0-beta-1076/crates/muxy-app/src/model/extensions/calls.rs#L691-L741) validates that placeholder and maps failure to the displayed error. The [protocol validator](https://github.com/muxy-app/muxy/blob/v2.0.0-beta-1076/crates/muxy-protocol/src/exec.rs#L20-L45) rejects a zero job ID. The request therefore fails before runtime consent or process execution. Run Deck's shell call conforms to the [documented exec API](https://muxy.app/docs/extensions/scripts) and already declares `commands:exec`; changing permissions, command quoting, or switching to argv cannot resolve this host preflight bug.

Native probes through the actual panel bridge returned the same error for all three requests:

- The existing 974-character scan, with no NUL characters, timeout 5000 ms.
- `muxy.exec({ shell: 'printf run-deck-probe', timeoutMs: 5000 })`.
- `muxy.exec(['/usr/bin/printf', 'run-deck-probe'], { timeoutMs: 5000 })`.

A temporary Rust harness compiled the release's unchanged `ExecRequest::validate` implementation (serialization derives removed, unused project/path types stubbed; no cwd supplied). For both shell and argv forms, job 0 produced `Err(BadRequest)` and changing only the job to 1 produced `Ok(())`. This isolates the conflicting preflight placeholder; it is not an end-to-end verification of a patched Muxy binary.

Upstream repair should separate payload validation from assigned-job validation, or use a nonzero placeholder solely for preflight while retaining real job assignment before dispatch. Host regression coverage should exercise consent preflight for both exec APIs, both command forms, and malformed requests. Run Deck has no seam that can repair or regression-test this host-owned placeholder. No permission bypass, binary modification, upstream issue, or PR was made.

After removing temporary instrumentation, `npm run check` passed **148 tests**, zero failures/skips, and distribution checks (116.2 KiB). Restored JS `index-DQWwtyXl.js`, SHA-256 `fc411450bc6b2da699922b0ba9cdc999b18c3662fe7f0b87361f31ee58cf9e15`; CSS `index-DUU9nb3u.css`, SHA-256 `b0186e18359eecd0ca9d08b9c9a88230333c8e125195f281e4bc828ee28122ab`.

Reloaded the restored build through Extensions, reopened the native panel, and repeated Refresh: the same host error remains, with no probe logging. No test service, terminal, saved command, or permission grant was created. Existing PrintAnvil services, the selected terminal/project, English language, and paused live mode were preserved. The exact blocked development step is **MUXY-DEVELOPMENT step 3: successful native Refresh**, followed by scan/Stop/Restart verification on a corrected Beta host. Muxy 1.6.0's earlier evidence below does not certify this Beta release.
